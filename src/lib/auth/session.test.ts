...
describe('session token', () => {
  it('round-trips the user id', async () => {
    const now = 1_000_000;
    const token = await createSessionToken('user-1', SECRET, now);
    expect(await verifySessionToken(token, SECRET, now + 10)).toEqual({ uid: 'user-1' });
  });
  ...
  it('rejects a wrong secret', async () => {
    const token = await createSessionToken('user-1', SECRET, 1000);
