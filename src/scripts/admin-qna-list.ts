...
function updateHiddenCount(): void {
  const count = document.querySelector<HTMLElement>('#admin-count-hidden');
  if (!count) return;
  const next = Math.max(0, Number(count.dataset.count ?? count.textContent ?? 0) - 1);
...
function showEmptyStateIfNeeded(): void {
  ...
  wrapper.className = 'border-b border-hairline py-12 text-center';
...
root?.querySelectorAll<HTMLButtonElement>('[data-restore-question]').forEach((button) => {
  button.addEventListener('click', async () => {
    ...
    if (!questionId || !Number.isSafeInteger(expectedRevision) || !csrfToken) return;
